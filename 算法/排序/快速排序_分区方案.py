...
if __name__ == "__main__":
    ...
    quicksort_hoare(test_arr)
    print(f"✅ Hoare分区: {original} → {test_arr}")
    
    test_arr = original.copy()
    quick_sort_iterative(test_arr)
    print(f"✅ 非递归版: {original} → {test_arr}")
    
    test_arr = original.copy()
    quicksort_3way(test_arr, 0, len(test_arr) - 1)
    print(f"✅ 三路分区: {original} → {test_arr}")