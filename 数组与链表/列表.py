...
class MyList:
    ...
    def insert(self, num: int, index: int):
        ...
        if self._size == self._capacity:
            self.extend_capacity()
        """后循环标准写法. 最后一位要保留下来, 所以只是self._size-1而不是-2"""
    ...
    def remove(self, index: int) -> int:
        ...
        '''应改成: if index < 0 or index >= self._size:'''
        # if index + 1 >= self._size:
        #     raise IndexError("越界")
        # delete = self.get(index)
        """self._size - 1 最后在 i + 1 处越界"""
    ...
    def extend_capacity(self):
        ...
        for i in range(self._capacity):
            self._arr.append(0)
        self._capacity = self._capacity * self._extend_ratio
